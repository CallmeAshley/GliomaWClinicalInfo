...
from transformers import AutoTokenizer, BertModel, AutoModel, CLIPTokenizer, CLIPTextModel, CLIPModel, BioGptModel
...
class ClinicalViT(VisionTransformer):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1,
        ...
        if self.clini_embed_token == 'cls':      # CLS a mri of an old patient SEP
            self.clini_embed_token_idx = 0
        elif self.clini_embed_token == 'word':   # CLS a mri of a female patient SEP
            # self.clini_embed_token_idx = 5
            self.clini_embed_token_idx = 6
        ...
        if clini_info_style == 'bert':
            ...
            self.clini_embedding = nn.Embedding(4, 768)
        elif clini_info_style == 'biobert':
            self.tokenizer = AutoTokenizer.from_pretrained("dmis-lab/biobert-base-cased-v1.1")   # dmis-lab/biobert-v1.1
            self.embed_model = BertModel.from_pretrained("dmis-lab/biobert-base-cased-v1.1")
        elif clini_info_style == 'clip':
            self.tokenizer = AutoTokenizer.from_pretrained("openai/clip-vit-large-patch14")   
            self.embed_model = CLIPTextModel.from_pretrained("openai/clip-vit-large-patch14")
        elif clini_info_style == 'biogpt':
            self.tokenizer = AutoTokenizer.from_pretrained("microsoft/biogpt")   
            self.embed_model = BioGptModel.from_pretrained("microsoft/biogpt")
            
            
            # self.projector = nn.Linear(1024, 768)
            # self.norm_proj = nn.LayerNorm(768, eps=1e-6)
            
            # trunc_normal_(self.projector.weight, std=.02)
    ...
    def forward(self, x, clinical_feats=None):   # [['43', '60', '42', '38', '67', '43', '70', '53', '69', ...], ['male', 'male', 'male', 'male', 'male', 'male', 'male', 'female', 'male', ...]]
        x = self.forward_features(x)     # x.shape: torch.Size([64, 197, 768])  ViT encoder 결과
        out = x[:, 0]   # out.shape: torch.Size([64, 768])
        
        if clinical_feats != None:
            age_embed, sex_embed = self.forward_clinical_embed(clinical_feats)   
            multi_modal_feats = torch.stack([out, age_embed, sex_embed], 1) # torch.Size([64, 3, 768])  3  --> 3개 concat  했으므로
            multi_modal_feats = multi_modal_feats + self.decoder_pos_embed  # decoder_pos_embed size: torch.Size([1, 3, 768])
            multi_modal_feats = self.decoder_blocks(multi_modal_feats)    # self attention
            out = self.decoder_norm(multi_modal_feats)
    
        out = self.forward_head(out)    # MLP Head
        return out
    ...
    def forward_clinical_embed(self, clinical_feats):
        ...
        age = ['an old' if int(i)>=45 else 'a young' for i in age]

        if self.clini_info_style == 'bert' or self.clini_info_style == 'biobert' or self.clini_info_style == 'clip' or self.clini_info_style == 'biogpt':
            # age_prompt = ['a medical image of a ' + age[i] + ' patient' for i in range(len(age))]
            # sex_prompt = ['a medical image of a ' + sex[i] + ' patient' for i in range(len(sex))]
            
            age_prompt = ['a mri of ' + age[i] + ' patient' for i in range(len(age))]      # 앞에 phrase를 argument로 줄 수 있도록... maybe model yaml에 추가하면 될 듯?
            sex_prompt = ['a mri of a ' + sex[i] + ' patient' for i in range(len(sex))]    # 사실 이미지에 달린 caption...이긴 한데... clip? 될까? text가 너무 고정적
        
            age_token = self.tokenizer(age_prompt, return_tensors="pt")
            ...
            if self.embed_trainable:
                ...
                self.embed_model.eval()
                with torch.no_grad(): #age_token['input_ids'].shape: torch.Size([64, 8])                     
                    age_embed = self.embed_model(**age_token).last_hidden_state[:, self.clini_embed_token_idx]    # cls token 가져올 건지 아니면 word token 가져올 건지
                    sex_embed = self.embed_model(**sex_token).last_hidden_state[:, self.clini_embed_token_idx]
                    
                    if self.clini_info_style == 'biogpt':
                        age_embed = self.projector(age_embed)
                        age_embed = self.norm_proj(age_embed)
                        sex_embed = self.projector(sex_embed)
                        sex_embed = self.norm_proj(sex_embed)
        ...
        return age_embed, sex_embed    # torch.Size([64, 768])
