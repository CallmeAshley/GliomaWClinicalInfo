...
class ResNet_multi4(nn.Module):

    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=1000, in_chans=1, base_ch=64, long= False, zero_init_residual=False):
        super(ResNet_multi4, self).__init__()
        self.inplanes = base_ch
        self.inc = nn.Sequential(Conv2d(in_chans, base_ch, kernel_size=7, stride=2, padding=3, bias=False),
                                 BatchNorm2d(base_ch),
                                 ReLU(inplace=True),
        ...
        # self.proj = DANetHead(512 * block.expansion, 512 * block.expansion//4)
                               
        self.cat = Cat()
        self.proj = Sequential(Linear(base_ch*8 * 4 * block.expansion, base_ch*8 * block.expansion),
                               BatchNorm1d(base_ch*8 * block.expansion),
                               ReLU(inplace=True)
                               )
    ...
    def forward(self, x, mode='output', target_class = [None]):
        ...
        layer0_t1 = self.inc(t1)
        layer1_t1 = self.layer1(layer0_t1)
        layer2_t1 = self.layer2(layer1_t1)
        layer3_t1 = self.layer3(layer2_t1)
        layer4_t1 = self.layer4(layer3_t1)
        
        
        layer0_t1c = self.inc(t1c)
        layer1_t1c = self.layer1(layer0_t1c)
        layer2_t1c = self.layer2(layer1_t1c)
        layer3_t1c = self.layer3(layer2_t1c)
        layer4_t1c = self.layer4(layer3_t1c)
        
        
        layer0_t2 = self.inc(t2)
        layer1_t2 = self.layer1(layer0_t2)
        layer2_t2 = self.layer2(layer1_t2)
        layer3_t2 = self.layer3(layer2_t2)
        layer4_t2 = self.layer4(layer3_t2)
        
        
        layer0_flair = self.inc(flair)
        layer1_flair = self.layer1(layer0_flair)
        layer2_flair = self.layer2(layer1_flair)
        layer3_flair = self.layer3(layer2_flair)
        layer4_flair = self.layer4(layer3_flair)
        
        z_t1 = self.global_pool(layer4_t1)
        z_t1 = z_t1.view(z_t1.size(0), -1)
        z_t1c = self.global_pool(layer4_t1c)
        z_t1c = z_t1c.view(z_t1c.size(0), -1)
        z_t2 = self.global_pool(layer4_t2)
        z_t2 = z_t2.view(z_t2.size(0), -1)
        z_flair = self.global_pool(layer4_flair)
        z_flair = z_flair.view(z_flair.size(0), -1)
        
        z = self.cat([z_t1, z_t1c, z_t2, z_flair], 1)
        z = self.proj(z)
        z = self.fc(z)
...
class ResNet_multi4_radiomics(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=1, base_ch=64, long= False, zero_init_residual=False):
        ...
        self.mixed_cat = Cat()
        self.mixed_proj = Sequential(Linear(base_ch*8 * block.expansion + radiomics_ch, base_ch*8 * block.expansion//4),
                               BatchNorm1d(base_ch*8 * block.expansion//4),
                               ReLU(inplace=True)
                               )
                               
        self.contrast_cat = Cat()
        self.proj = Sequential(Linear(base_ch*8 * block.expansion, base_ch*8 * block.expansion),
                               BatchNorm1d(base_ch*8 * block.expansion),
                               ReLU(inplace=True)
                               )
    ...
    def forward(self, x, radiomics):
        ...
        t1_radi = radiomics[:, 0]
        t1c_radi = radiomics[:, 1]
        t2_radi = radiomics[:, 2]
        flair_radi = radiomics[:, 3]
        
        z_t1 = self.mixed_cat([z_t1, t1_radi], 1)
        z_t1c = self.mixed_cat([z_t1c, t1c_radi], 1)
        z_t2 = self.mixed_cat([z_t2, t2_radi],1 )
        z_flair = self.mixed_cat([z_flair, flair_radi], 1)
        
        z_t1 = self.mixed_proj(z_t1)
        z_t1c = self.mixed_proj(z_t1c)
        z_t2 = self.mixed_proj(z_t2)
        z_flair = self.mixed_proj(z_flair)
        
        z = self.contrast_cat([z_t1, z_t1c, z_t2, z_flair], 1)
        z = self.proj(z)
        z = self.fc(z)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=1, base_ch=64, long= False, zero_init_residual=False, hook=False):
...
def resnet10_multi4(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_multi4(BasicBlock, [1, 1, 1, 1], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet10_multi4_radiomics(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_multi4_radiomics(BasicBlock, [1, 1, 1, 1], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model
...
def resnet18_multi4(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_multi4(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model

def resnet18_multi4_radiomics(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_multi4_radiomics(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet50_multi4(pretrained=False, long = False,**kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_multi4(Bottleneck, [3, 4, 6, 3], long = long,**kwargs)
    if pretrained:
