...
class Brain_Dataset(Dataset):
    ...
    def patient_and_class_selection(self):
        ...
        df_from_excel.replace(['M'], 'male', inplace=True)
        df_from_excel.replace(['F'], 'female', inplace=True)
...
def train(criterion, optimizer, net, epoch, train_loader, writer, scaler, cfg, distributed):
    ...
    for batch_idx, data_dict in enumerate(train_loader):
        inputs = data_dict['image'].cuda(non_blocking=True).float()  # [64, 4, 224, 224]
...
def val(criterion, net, epoch, val_loader, writer, cfg, distributed): 
    ...
    with torch.no_grad():
        for batch_idx, data_dict in enumerate(val_loader):
            inputs = data_dict['image'].cuda(non_blocking=True).float()    #  torch.Size([64, 4, 224, 224])
            targets = data_dict['label'].cuda(non_blocking=True)
            clinical_feats = data_dict['clinical_feats']
            ...
            if cfg.trainer.amp:
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    # outputs = net(inputs)
                    outputs = net(inputs, clinical_feats)      # ([64, 4, 224, 224])
                        
                    loss = criterion(outputs.squeeze(1), targets)    #  [64, 1]
            else:
                # outputs = net(inputs)
                outputs = net(inputs, clinical_feats)
