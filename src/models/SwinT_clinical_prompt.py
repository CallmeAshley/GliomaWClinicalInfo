...
class ClinicalSwinT(SwinTransformer):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1, window_size=7,
        ...
        if self.info_mode == 'both':
            self.decoder_pos_embed = nn.Parameter(torch.randn(1, 3, self.num_features) * .02)
            trunc_normal_(self.decoder_pos_embed, std=.02)
        else:
            self.decoder_pos_embed = nn.Parameter(torch.randn(1, 2, self.num_features) * .02)
            trunc_normal_(self.decoder_pos_embed, std=.02)
    ...
    def forward(self, x, clinical_feats):
        ...
        elif self.info_mode == 'age':
            age_embed = self.forward_clinical_embed(clinical_feats)
            
            x = self.forward_features(x)
            x = self.forward_head(x, pre_logits=True)
            multi_modal_feats = torch.stack([x, age_embed], 1)
            
        else:
            sex_embed = self.forward_clinical_embed(clinical_feats)
            
            x = self.forward_features(x)
            x = self.forward_head(x, pre_logits=True)
            multi_modal_feats = torch.stack([x, sex_embed], 1)
    ...
    def forward_clinical_embed(self, clinical_feats):
        if self.info_mode == 'both':
            ...
            if self.clini_info_style == 'bert':
                age_prompt = ['a mri of ' + age[i] + ' patient' for i in range(len(age))]
                sex_prompt = ['a mri of a ' + sex[i] + ' patient' for i in range(len(sex))]
