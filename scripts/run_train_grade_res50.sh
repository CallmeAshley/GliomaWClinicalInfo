...
random_seed="2345" # 51 server, grade seed
...
# single_ckpt='/mai_nas/LSH/glioma_with_clinical/exp/runs/grade/20240117/125928/train/checkpoint_best_auc.pth'
...
# single_ckpt='/mai_nas/LSH/glioma_with_clinical/exp/runs/grade/20240117/125928/train/checkpoint_best_loss.pth'
eval_time=$(date "+%Y%m%d")"_"$(date "+%H%M%S")
python eval_internal.py task_name=val ckpt=$single_ckpt paths.save_dir=$save_dir paths=internal_eval paths.time_dir=$eval_time \
            metric=$metric \
            model.num_classes=$num_classes \
